...
test("#2454 Opus 5 full-agent omits context-1m but keeps mid-conversation-system", () => {
  const flags = selectBetaFlags(fullAgentBody("claude-opus-5"));
  assert.ok(!flags.includes("context-1m-2025-08-07"), "Opus 5 should NOT receive context-1m");
  assert.ok(
    flags.includes("mid-conversation-system-2026-04-07"),
    "Opus 5 should receive mid-conversation-system"
  );
});
